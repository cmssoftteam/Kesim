...
typedef enum McExclusionCmdEnum
{	mcEXCL_GET_STATUS,
	mcEXCL_EXCLUDE,
	mcEXCL_INCLUDE
} McExclusionCmdEnum;
...
typedef enum McAGSRQSPUseSrcEnum
{	mcAGSRQSPUS_VAR = 0,
	mcAGSRQSPUS_IO_CH = 1
} McAGSRQSPUseSrcEnum;

typedef enum McAGSRQSPUseLvlEnum
{	mcAGSRQSPUL_LOW = 0,
	mcAGSRQSPUL_HIGH = 1
} McAGSRQSPUseLvlEnum;

typedef enum McAGSRQSPwrOffAStopEnum
...
typedef struct McGroupQuickstopInfoType
{	plcbit IsActive;
...
typedef struct McExclusionParType
...
typedef struct McAGSRQSPUseSrcVarType
{	plcstring PVMapping[251];
} McAGSRQSPUseSrcVarType;

typedef struct McAGSRQSPUseSrcIOChType
{	plcstring ChannelMapping[251];
} McAGSRQSPUseSrcIOChType;

typedef struct McAGSRQSPUseSrcType
{	enum McAGSRQSPUseSrcEnum Type;
	struct McAGSRQSPUseSrcVarType Variable;
	struct McAGSRQSPUseSrcIOChType IOChannel;
} McAGSRQSPUseSrcType;

typedef struct McAGSRQSPUseType
{	struct McAGSRQSPUseSrcType Source;
	enum McAGSRQSPUseLvlEnum Level;
} McAGSRQSPUseType;

typedef struct McAGSRQSPwrOnAStopType
{	enum McAGSRQSPwrOnAStopEnum Type;
	struct McAGSRQSPUseType Used;
} McAGSRQSPwrOnAStopType;

typedef struct McAGSRQSPwrOffAStopType
{	enum McAGSRQSPwrOffAStopEnum Type;
	struct McAGSRQSPUseType Used;
} McAGSRQSPwrOffAStopType;

typedef struct McAGSRQSType
...
typedef struct MC_BR_SkipBlock
{
	/* VAR_INPUT (analog) */
	struct McAxesGroupType* AxesGroup;
	/* VAR_OUTPUT (analog) */
...
typedef struct MC_BR_GroupAxisExclusion
{
	/* VAR_INPUT (analog) */
	struct McAxesGroupType* AxesGroup;
	enum McExclusionCmdEnum Command;
	enum McExclusionTypeEnum Type;
	struct McExclusionParType Parameters;
	enum McExecutionModeEnum ExecutionMode;
	/* VAR_OUTPUT (analog) */
	signed long ErrorID;
	/* VAR (analog) */
